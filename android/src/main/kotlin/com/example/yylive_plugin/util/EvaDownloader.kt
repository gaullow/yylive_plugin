package com.example.yylive_plugin.util
...
class EvaDownloader(context: Context?) {
    ...
    //李利明添加
    interface ParseImageCompletion {
...
//                    }
    ...
    //李利明添加
    fun decodeImageFromURL(
        url: URL,
        callback: ParseImageCompletion?,
    ): (() -> Unit)? {
        threadPoolExecutor.execute {
            //获得HttpURLConnection对象
            val coon = url.openConnection() as HttpURLConnection
            coon.doInput = true //允许输入流，即允许下载
            coon.useCaches = false //不使用缓冲
            try {
                coon.requestMethod = "GET" //使用get请求
                val `in` = coon.inputStream //获得图片的输入流
                val bout = ByteArrayOutputStream() //
                val buff = ByteArray(1024) //定义缓冲区
                var len = -1
                //不断地从输入流读取字节数组
                while (`in`.read(buff).also { len = it } != -1) {
                    bout.write(buff, 0, len)
                }
                val arr = bout.toByteArray() //字节数组输出流转换为字节数组
                var pic = BitmapFactory.decodeByteArray(arr, 0, arr.size)
                `in`.close()
                callback?.onComplete(pic);
            } catch (e: Exception) {
                callback?.onError()
            }
